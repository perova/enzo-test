...
! function( $ ) {
	...
	$.fn.onepage_scroll = function( options ) {
		var settings                                      = $.extend( {}, defaults, options ), el = $( this ), sections = $( this )
			.children( settings.sectionContainer ), total = sections.length, status = "off", topPos = 0, leftPos = 0, lastAnimation = 0, quietPeriod = 500, paginationList = "";
		...
		$.fn.transformPage = function( settings, pos, index ) {
			...
			var $current = $( sections[index - 1] );
			if ( $current.hasClass( 'section-auto-height' ) ) {
				var footer_height = $current.outerHeight();
		...
		if ( settings.pagination == true ) {
			if ( $( 'ul.onepage-pagination' ).length < 1 ) {
				$( "<ul class='onepage-pagination'></ul>" )
					.prependTo( "body" );
			}

			if ( settings.direction == 'horizontal' ) {
				posLeft = (
							  el.find( ".onepage-pagination" ).width() / 2
						  ) * - 1;
				el.find( ".onepage-pagination" ).css( "margin-left", posLeft );
			} else {
				posTop = (
							 el.find( ".onepage-pagination" ).height() / 2
						 ) * - 1;
